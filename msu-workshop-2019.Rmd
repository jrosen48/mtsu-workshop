...
1. Questions and sundry tools (time permitting)
...
To find a list of all those functions, you can run this in the R Studio console:
...
We can also rename the variables at the same time we select them. I put these on separate lines so I could add the comment, but you could do this all in the same line, too. It does not make a difference in terms of how `select()` will work. We are using `head()` to only print a few rows.
...
It is very flexible ... but can also be a bit challenging to use, at first.
